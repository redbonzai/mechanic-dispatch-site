  ...
  template: `
    <div class="auth-page">
      <div class="auth-card">
        ...
        <p class="auth-footer">No account? <a routerLink="/mechanic-register">Register here →</a></p>
        <p class="auth-footer">Driver? <a routerLink="/login">Driver sign in →</a></p>
      </div>
    </div>
  `,
  styles: [`
    ...
    .submit-btn { width: 100%; padding: 12px; background: #059669; color: #fff; border: none; border-radius: 10px; font-size: 15px; font-weight: 700; cursor: pointer; margin-top: 8px; transition: background 0.15s; &:hover:not(:disabled) { background: #10b981; } &:disabled { opacity: 0.6; cursor: not-allowed; } }
...
export class MechanicLoginComponent {
  ...
  submit() {
    ...
    this.authService
      ...
      .subscribe({
        next: () => void this.router.navigate(['/mechanic-dashboard']),
        error: (err) => {
