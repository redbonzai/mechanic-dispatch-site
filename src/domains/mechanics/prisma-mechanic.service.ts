...
@Injectable()
export class PrismaMechanicService implements MechanicAbstract {
  ...
  async findBySlug(slug: string): Promise<Mechanic | null> {
    const mechanic = await this.prisma.mechanic.findUnique({
      where: { slug },
    });
  ...
  async findMany(params?: { isActive?: boolean }): Promise<Mechanic[]> {
  ...
  private mapToDomain(record: MechanicRecord): Mechanic {
